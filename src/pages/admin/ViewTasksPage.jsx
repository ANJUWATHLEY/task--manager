...
const taskDelete = async (taskId) => {
  ...
  const handleTaskPriority = async (taskId, level) => {
    try {
      await axios.put(`/admin/priority/${taskId}`, { priority: level , REFTASK: REFTASK }, {
        headers: { Authorization: `Bearer ${token}` },
          ...
          {filteredTasks.map((task) => (
            <li key={task.id} className="bg-white rounded-xl shadow-md p-5 hover:shadow-xl transition">
              ...
              <div className="flex gap-3 mt-4">
                <button   onClick={() => navigate(`/admin/update-task/${task.id}`)}  className="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-2 rounded-lg text-sm flex items-center gap-1">
